...
const LEVELS = [...CHAPTER1, ...CHAPTER2];

test('구현된 LEVEL은 1~20까지 빠짐없이 이어진다', () => {
...
test('모든 LEVEL과 문제가 검증을 통과한다', () => {
  ...
  assert.ok(problems.length >= 80, `문제 수가 ${problems.length}개뿐입니다`);
});
...
test('오답에는 반드시 이유가 붙는다 (요구사항 79)', () => {
  ...
  assert.ok(checked > 40, `오답 설명을 확인한 문제가 ${checked}개뿐입니다`);
});
...
test('공식 카드는 네 단계(공식·왜·언제·예외)를 모두 갖춘다 (요구사항 51)', () => {
  ...
  assert.ok(withFormula.length >= 10, '공식 카드가 너무 적습니다');
